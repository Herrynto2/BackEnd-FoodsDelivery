...
module.exports = {
    ...
    get: (id, params) => {
        ...
        if (id) {
            return new Promise((resolve, reject) => {
                conn.query(`SELECT * FROM user where id = ${id}`, (error, results, fields) => {
...
//             //Count All data 
//             conn.query(`SELECT COUNT(*) AS total from user ${condition.slice(0, condition.indexOf('LIMIT'))}`, (error, results, fields) => {
...
//                 }
//                 const { total } = results[0]
//                 const query = `SELECT * FROM user ${condition}`
